...
public class LibraryMain {
	...
	private static Customer getCustomer(Session session, String cName) {
		...
		SQLQuery query = session.createSQLQuery("select * from Customer where customer_name = "
				+ "'" + cName+"'");
	...
	private static Book getBook(Session session, String bName) {
		// TODO Auto-generated method stub
		SQLQuery query = session.createSQLQuery("select * from Book where book_name = "
				+ "'" + bName+"'");
		List boo=query.list();
	/*	Employee e=(Employee)emp.get(0);*/
	...
	public static Book addbook(BufferedReader br) throws IOException {
	 ...
	 public static void return_Book(Session session,Customer c,Book b,BufferedReader br) throws NumberFormatException, IOException
	 {
	...
	if(fine>0)
	  ...
	  if(fineCollected==1)
	  {
	   String query1="update book_issue set no_of_days="+noOfDays
			   +" where cust_customer_id = "+c.getCustomerId()+"  and book_book_id = "+b.getBookId();
